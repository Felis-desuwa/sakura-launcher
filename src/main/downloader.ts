...
export async function startDownload(rawUrl: string, intoDir?: string): Promise<StartResult> {
  ...
  if (!dir) return { ok: false, error: '还没有设置下载目录，请先在设置里指定，或先添加一个游戏库文件夹' }
  try {
    fs.mkdirSync(dir, { recursive: true })
  } catch (err) {
    return { ok: false, error: `无法创建下载目录：${err instanceof Error ? err.message : err}` }
  }
  ...
  if (!command && key !== 'system') {
    return {
      ok: false,
      error: key === 'idm' ? '没有找到 IDM，请在设置里指定 IDMan.exe' : '请先在设置里指定下载器程序'
    }
  }
  ...
  if (entry.downloader === 'aria2') {
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        update(entry.id, { status: 'failed', message: stderr.trim() || `aria2c 退出码 ${code}` })
  ...
  child.on('close', (code) => {
    ...
    if (code !== 0 && entry.downloader === 'custom') {
      ...
      if (still?.status === 'downloading') {
        update(entry.id, { status: 'failed', message: `下载器退出码 ${code}` })
        live.delete(entry.id)
      }
    }
  })
...
function tick(): void {
  for (const entry of db.getDownloads()) {
    ...
    if (Date.now() - entry.startedAt > WATCH_TIMEOUT_MS) {
      update(entry.id, { status: 'failed', message: '等待超过 12 小时，已停止监视' })
      live.delete(entry.id)
      continue
    }
...
function settle(id: string, trustProcess: boolean): void {
  ...
  if (!first) {
    ...
    update(id, {
      ...
      percent: 100,
      volumes: files,
      message: `已下载 ${files[0]}，不是压缩包，未自动解压`
    })
    live.delete(id)
    return
  }
  ...
  const extract = extractArchive(
    ...
    (result) => {
      ...
      if (!result.ok) {
        update(id, { status: 'failed', message: result.error ?? '解压失败' })
        live.delete(id)
        return
      }
...
async function finishImport(
  ...
  let note = added > 0 ? `已导入 ${added} 个游戏` : '解压完成，但没有识别出游戏，可用「导入文件夹」手动挑选'
...
export function resumeDownloads(): void {
  ...
  for (const entry of db.getDownloads()) {
    if (entry.status === 'downloading') {
      ...
      db.upsertDownload({ ...entry, status: 'failed', message: '解压过程被中断，请重新解压压缩包' })
      touched = true
    }
  }
