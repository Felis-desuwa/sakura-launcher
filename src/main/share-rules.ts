...
function judgeFile(name: string, depth: number, insideSave: boolean): Match | null {
  ...
  if (LAUNCHER_FILES.has(lowered)) {
    return { category: 'launcher', reason: t('shareWhy.launcher') }
  }

  // Never a candidate, no matter what other rule might reach for it.
  if (GAME_DATA_EXT.has(ext)) return null

  if (SAVE_EXT.has(ext)) return { category: 'save', reason: t('shareWhy.saveExt', { ext }) }
  if (insideSave && SAVE_EXT_SCOPED.has(ext)) {
    return { category: 'save', reason: t('shareWhy.saveInDir', { ext }) }
  }
  // NScripter and KiriKiri drop save1.dat, save2.dat… straight in the game folder.
  if (depth === 0 && /^save\d*\.(dat|bin)$/.test(lowered)) {
    return { category: 'save', reason: t('shareWhy.saveRoot') }
  }

  if (NOISE_NAMES.has(lowered)) return { category: 'noise', reason: t('shareWhy.noiseName') }
  if (NOISE_EXT.has(ext)) return { category: 'noise', reason: t('shareWhy.noiseExt', { ext }) }
  ...
  if (depth === 0 && (CONFIG_NAMES.has(lowered) || CONFIG_EXT.has(ext))) {
    return { category: 'config', reason: t('shareWhy.config') }
  }
      ...
      if (entry.isDirectory()) {
        ...
        if (save || noise) {
          found.push({
            ...
            reason: save ? t('shareWhy.saveDir') : t('shareWhy.noiseDir'),
            checked: true
          })
