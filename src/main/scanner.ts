...
export interface RescanOptions {
   ...
   * games is a deliberate act with a preview attached to it (Import a folder / Rescan and add),
...
export function rescan(options: RescanOptions = {}): ScanOutcome {
  ...
  // Left alone it gets skipped by every scan and then reported as missing — present, yet
  ...
  for (const root of settings.roots) {
    ...
    for (const found of result.games) {
      ...
      next.push({
        ...
        tags: prev?.tags ?? [],
        // Auto tags survive a rescan even though the folder changed. Working them out is
        // an explicit act the user asked for — quietly clearing them because a file moved
        // would read as the scan having thrown their work away.
        autoTags: prev?.autoTags ?? [],
...
export function previewFolder(folder: string): ImportPreview {
  ...
  // rescan — would undo their decision for them. Settings → Removed entries is where those
  // live, with Restore to take one back and Forget to have it offered here again.
  const known = new Set([
...
function describeReason(reason: string): string {
  ...
  if (reason.startsWith('payload too small')) {
    return t('scan.tooSmall', {
      size: size?.[1] ?? '?',
      extra: size?.[2] ? t('scan.tinyExe') : ''
    })
  }
  if (reason === 'archive staging folder') return t('scan.staging')
  return reason
}
...
export function importFolder(
  ...
  // missing on the very next refresh.
...
export function addGameByExe(exePath: string, extras: AddGameExtras = {}): Game | null {
  ...
  // as missing — present in the library, skipped by every scan.
  const settings = db.getSettings()
