...
import { ENGINE_META, type EngineId } from '../shared/types.ts'
...
export interface RuntimePackage {
  key: string
  /** What to go and install, by the name it is distributed under. */
  label: string
  note: string
  test: RegExp
}
...
export const RUNTIME_PACKAGES: RuntimePackage[] = [
  {
    key: 'vc2015',
    label: 'Visual C++ 2015-2022 运行库',
    note: '微软官方下载页叫「Visual C++ Redistributable」，装 x86 与 x64 两个版本最省事',
    // msvcp140_1、msvcp140_atomic_wait、vcruntime140_1 都属于同一份可再发行组件。
    test: /^(msvcp140|vcruntime140|concrt140|mfc140u?|vcomp140)(_[a-z0-9_]+)?\.dll$/i
  },
  {
    key: 'vc2013',
    label: 'Visual C++ 2013 运行库',
    note: '版本号 12.0',
    test: /^(msvcp120|msvcr120|mfc120u?|vcomp120)\.dll$/i
  },
  {
    key: 'vc2012',
    label: 'Visual C++ 2012 运行库',
    note: '版本号 11.0',
    test: /^(msvcp110|msvcr110|mfc110u?)\.dll$/i
  },
  {
    key: 'vc2010',
    label: 'Visual C++ 2010 运行库',
    note: '版本号 10.0 —— 这个年代的日系游戏最常缺它',
    test: /^(msvcp100|msvcr100|mfc100u?)\.dll$/i
  },
  {
    key: 'vc2008',
    label: 'Visual C++ 2008 运行库',
    note: '版本号 9.0',
    test: /^(msvcp90|msvcr90|mfc90u?)\.dll$/i
  },
  {
    key: 'vc2005',
    label: 'Visual C++ 2005 运行库',
    note: '版本号 8.0',
    test: /^(msvcp80|msvcr80|mfc80u?)\.dll$/i
  },
  {
    key: 'directx',
    label: 'DirectX 末端用户运行库（DirectX End-User Runtime）',
    note: '系统自带的 DirectX 不含这些旧组件，必须单独装一次，装完是永久的',
  ...
  },
  {
    ...
    note: 'N 版或 KN 版 Windows 缺少媒体组件，视频过场会让游戏直接起不来',
    test: /^(mfplat|mfreadwrite|mfcore|evr)\.dll$/i
  },
  {
    key: 'openal',
    label: 'OpenAL',
    note: '游戏目录里通常自带 oalinst.exe',
    test: /^(openal32|wrap_oal)\.dll$/i
  },
  {
    key: 'physx',
    label: 'NVIDIA PhysX 系统软件',
    note: '',
...
export function localeVerdict({ acp, engine, names }: LocaleSignals): LocaleVerdict {
  ...
  if (mojibake.length > 0) {
    reasons.push(`文件名已经是乱码的：${mojibake.slice(0, 3).join('、')}`)
  }
  if (engine && JP_ERA_ENGINES.has(engine)) {
    reasons.push(`${ENGINE_META[engine].label} 是按日文系统写的引擎，走的是 ANSI 接口`)
  }
  if (names.some(hasKana)) {
    reasons.push('文件名里有日文假名')
  }
  if (acp !== null && acp !== JAPANESE_ACP) {
    reasons.push(`当前系统非 Unicode 程序用的是代码页 ${acp}，不是日文的 932`)
  }
