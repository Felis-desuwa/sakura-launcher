import { makeT, type Lang, type MessageKey } from '../../../shared/i18n'

/**
 * These are plain functions, not components, so they cannot reach the language through a
 * hook. `LangProvider` sets this during its render — before any child renders — which is
 * what makes the very first paint after a language change already correct.
 */
let t: (key: MessageKey) => string = makeT('zh')
...
export function formatBytes(bytes: number | null | undefined): string {
  if (bytes === null || bytes === undefined) return t('fmt.calculating')
...
export function formatDate(ts: number | null): string {
  if (!ts) return t('fmt.neverLaunched')
  const d = new Date(ts)
