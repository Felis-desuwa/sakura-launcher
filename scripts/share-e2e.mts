...
const gameDir = path.join(root, 'Test Game 日本語')
...
console.log('\n[3] 包内容')
const has = (rel: string): boolean => inside.some((p) => p === `Test Game 日本語/${rel}`)
...
check('顶层是游戏文件夹本身', inside.some((p) => p === 'Test Game 日本語'), inside.join(','))
