...
check('没设过读回来还是没设过', parseSidecar(renderSidecar(full)).upscale === undefined)

const withMode = parseSidecar(renderSidecar({ upscale: true, upscaleMode: 'Anime4K' }))
check('指定了模式就写模式名', /- 超分放大: Anime4K/.test(renderSidecar({ upscale: true, upscaleMode: 'Anime4K' })))
check('模式往返', withMode.upscale === true && withMode.upscaleMode === 'Anime4K')

/* 用户在 Magpie 自己界面里新建的模式，名字是他起的，这一行必须原样带得走——
   文件夹换台机器打开时，那边的 Magpie 能不能认出这个名字是那边的事 */
const own = parseSidecar(renderSidecar({ upscale: true, upscaleMode: 'CuNNy-8x32-NVL' }))
check('自建模式往返', own.upscale === true && own.upscaleMode === 'CuNNy-8x32-NVL')

/* 这个字段以前存的是内部 key，早先写出去的 sidecar 现在还在硬盘上 */
check(
  '旧 key 写出去时换成模式名',
  /- 超分放大: Integer Scale 2x/.test(renderSidecar({ upscale: true, upscaleMode: 'integer2x' }))
)

const onOnly = parseSidecar(renderSidecar({ upscale: true }))
check('开着但没指定模式', onOnly.upscale === true && onOnly.upscaleMode === undefined)

const offOnly = parseSidecar(renderSidecar({ upscale: false }))
check('关掉这一个', offOnly.upscale === false && offOnly.upscaleMode === undefined)
check('关掉写的是「关闭」', /- 超分放大: 关闭/.test(renderSidecar({ upscale: false })))
...
check('英文写法的模式名', enMode.upscale === true && enMode.upscaleMode === 'Integer Scale 2x')
check('中文字段名配模式名', parseSidecar('- 超分放大: CRT-Geom\n').upscaleMode === 'CRT-Geom')

/* 认不出的名字不再当作错误——Magpie 里能新建模式，这边没有名单可查。代价止于
   「Magpie 找不到这个模式，用它自己的默认」，不会波及同一个文件里别的字段 */
const unknown = parseSidecar('- 显示名称: 示例游戏\n- 超分放大: Anime8K\n')
check('没见过的模式名照样带着走', unknown.upscale === true && unknown.upscaleMode === 'Anime8K')
...
check('长得离谱的当这行不存在', tooLong.upscale === undefined && tooLong.name === '示例游戏')

/* 「这行没写」和「这行写了但读不懂」必须分得开：前者是人删掉这行、明确表示「跟随设置」，
   要把库里存着的答案清掉；后者只是打错字，什么都不该动。压成同一个值的话，一个笔误就会
   把这个游戏的设置悄悄重置 */
check('读不懂的一行留下记号', tooLong.upscaleUnreadable === true)
check('压根没写的不留记号', parseSidecar('- 显示名称: 示例游戏\n').upscaleUnreadable === undefined)
