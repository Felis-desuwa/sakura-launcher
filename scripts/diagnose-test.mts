...
check('中文名不是乱码', !looksLikeMojibake('穢翼的尤斯蒂娅'))
check('正常日文名不是乱码', !looksLikeMojibake('穢翼のユースティア.exe'))
...
check('假名识别', hasKana('ユースティア'))
check('汉字不算假名', !hasKana('穢翼'))
...
check(
  '只有日文名，不够',
  !localeVerdict({ acp: 936, engine: 'unity', names: ['ユースティア.exe'] }).needed
)
check(
  '老引擎 + 日文名 = 建议',
  localeVerdict({ acp: 936, engine: 'bgi', names: ['ユースティア.exe'] }).needed
)
check(
  '乱码 + 老引擎 = 建议',
...

{
  ...
  const gameDir = path.join(root, 'ユースティア')
