 ...
 *
 * The main fixture is drawn from a real release of 穢翼のユースティア, whose folder holds
 * twelve executables — engine, two uninstallers, a dated translation patch, a NoDVD
 * build, two settings tools and a locale emulator. Exactly the case a user cannot
 * resolve by looking. The listing is written out here rather than read from disk, so
 * the test needs no game installed and names nobody's library.
 */
import {
  classifyExes,
  exeKindOf,
...
const expected: [string, ExeKind][] = [
  ...
  ['ESUforEustia.exe', 'tool'],
  ['ISESUforEustia.exe', 'tool'],
  ['PCInformationViewer.exe', 'tool'],
  ['BGI_CHS_130321.exe', 'patch'],
  ['穢翼のユースティア NoDVD.EXE', 'patch'],
...
const ranked = rankExes(EUSTIA_DIR, eustia)
